    ...
    return (
        <div className="neo-card p-6">
            <h2 className="text-xl font-black text-[var(--text-main)] mb-6 flex items-center gap-2">
                <span className="text-indigo-500">📦</span> {t('resourceList.kv.title')}
            </h2>
            <div className="flex gap-2 mb-6">
                <input
                    value={newName}
                    onChange={e => setNewName(e.target.value)}
                    placeholder={t('resourceList.kv.enterName')}
                ...
                <div className="space-y-3">
                    ...
                    {resources.length === 0 && (
                        <div className="text-center py-10 opacity-50">
                            ...
                            <div className="text-[var(--text-muted)] font-medium">{t('resourceList.kv.noResources')}</div>
                        </div>
                    )}
                </div>
            )}
