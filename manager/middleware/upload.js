...
// 文件大小限制（字节）
const MAX_FILE_SIZE = DEFAULT_PROJECT_LIMITS.uploadMb * 1024 * 1024;
...
const createUpload = maxFileSize => multer({
    storage,
    limits: {
...
function resolveUploadLimitMb({ projectId, project, requestedUploadMb }) {
    if (projectId && !project) {
        const error = new Error('Project not found');
        error.statusCode = 404;
        throw error;
    }
    if (project) return project.limits.uploadMb;
    if (requestedUploadMb !== undefined) {
        return normalizeProjectLimits({ uploadMb: requestedUploadMb }).uploadMb;
    }
    return DEFAULT_PROJECT_LIMITS.uploadMb;
}
    ...
    if (err instanceof multer.MulterError) {
        if (err.code === 'LIMIT_FILE_SIZE') {
            return res.status(413).json({
                error: `文件大小超过限制（最大 ${(req.uploadMaxFileSize || MAX_FILE_SIZE) / 1024 / 1024}MB）`
            });
        }
...
upload.singleForProject = fieldName => (req, res, next) => {
    const projectId = req.get('x-project-id');
    const project = projectId ? projectService.getById(projectId) : null;
    let uploadMb;
    try {
        uploadMb = resolveUploadLimitMb({
            projectId,
            project,
            requestedUploadMb: req.get('x-project-upload-limit-mb')
        });
    } catch (error) {
        return res.status(error.statusCode || 400).json({ error: error.message });
    }
    const maxFileSize = uploadMb * 1024 * 1024;
    req.uploadProject = project;
    req.uploadMaxFileSize = maxFileSize;
    createUpload(maxFileSize).single(fieldName)(req, res, error => {
        if (error) return handleUploadError(error, req, res, next);
        next();
    });
};

upload.singleSafe = fieldName => (req, res, next) => {
    req.uploadMaxFileSize = MAX_FILE_SIZE;
    createUpload(MAX_FILE_SIZE).single(fieldName)(req, res, error => {
        if (error) return handleUploadError(error, req, res, next);
        next();
    });
};
...
module.exports.resolveUploadLimitMb = resolveUploadLimitMb;
