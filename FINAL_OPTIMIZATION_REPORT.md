...
> 历史报告（2026-04-08）。当前实现、测试命令和公网部署边界以 `README.md`、`docs/compatibility.md`、`docs/deployment.md` 及仓库 CI 为准；本文中的后续建议不代表当前未完成项。

## 当前状态（2026-07-31）

本报告中的早期建议已按当前实现重新核对。测试阶段、SQLite 控制面、审计日志、单管理员模型、资源回收站、不可变发布与隔离运行时已经落地，不应再作为待办事项重复实施。

当前验证结果：

- `./scripts/test-all.sh`：后端 `145/145`，前端 `26/26`，覆盖率、Lint、TypeScript、构建、依赖审计和 Compose 校验通过。
- `./scripts/test-runtime-broker.sh`：项目网络、文件系统、D1/KV/R2 隔离、构建磁盘限制和构建超时通过。
- `./scripts/test-e2e.sh`：Playwright `9/9` 通过，覆盖上传、立即发布、回滚、资源回收站、响应式项目表和持久化部署日志。

剩余建议见下方“后续建议”，仅列出当前确实值得投入的优化。
...
### P1：优先处理
1. 拆分约 550 KB 的前端主 bundle，按资源管理、项目 IDE 和系统设置做懒加载。
2. 扩展前端共享模块的真实分支覆盖率，并将覆盖率门禁接入 CI 合并保护。
3. 固定并定期刷新 Browserslist 数据，避免构建目标和实际浏览器支持范围漂移。

### P2：按规模投入
1. 若未来运行多个管理器实例，将项目操作协调器迁移到 SQLite 租约或 Redis，避免进程内锁失效。
2. 增加 Wrangler 行为兼容性测试矩阵，覆盖绑定错误、路由优先级、环境变量和静态资源缓存语义。
3. 为资源回收、部署和运行时限制增加指标告警与保留周期配置，而不是只依赖日志排查。

### 明确不建议
多管理员、跨项目共享资源、直接删除资源、恢复资源绑定、放开生产进程运行时，均与当前产品边界冲突，不应作为后续功能加入。
...
当前是否可公网部署，请以 `docs/deployment.md` 的密钥、域名、Caddy/TLS 和 Docker 检查清单为准；本历史报告不替代发布验收。
